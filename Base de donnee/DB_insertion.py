...
for fichier in fichiers_Classes_csv:
    ...
    for i in range(taille[fichier]):
        if not math.isnan(tab['rang_classe'][i]) or not math.isnan(tab['total_points'][i]) or not math.isnan(tab['moyenne_generale'][i]):
            c.execute(req_resultat, (int(tab['scei'][i]), float(tab['rang_classe'][i]), float(tab['total_points'][i].replace(',','.')), float(tab['moyenne_generale'][i].replace(',','.'))) )
...
for fichier in ['CMT_Oraux_YYYY_MP.xlsx', 'CMT_Oraux_YYYY_PC.xlsx']:
    ...
    for i in range(taille[fichier]):
        if (0<=tab['Maths'][i] and tab['Maths'][i]<=20):
            c.execute(req_oral_A, (int(tab["Numéro d'inscription"][i]), float(tab['Maths'][i]),float(tab['Phys.'][i]), float(tab['Entretien'][i]), float(tab['Anglais'][i])) )
...
for fichier in ['CMT_Oraux_YYYY_PSI.xlsx', 'CMT_Oraux_YYYY_PT.xlsx']:
    ...
    for i in range(taille[fichier]):
        if (0<=tab['Maths'][i] and tab['Maths'][i]<=20):
            c.execute(req_oral_A, (int(tab["Numéro d'inscription"][i]), float(tab['Maths'][i]), float(tab['S.I.'][i]), float(tab['Entretien'][i]), float(tab['Anglais'][i])) )
        if type(tab['Centre'][i]) != float and file[fichier]['Centre'][i] != ' ':
            c.execute(req_centre_jury, (int(tab["Numéro d'inscription"][i]), centre(tab['Centre'][i],tab['Jury'][i])) )
...
for i in range(taille['Classes_MP_CMT_spe_XXXX.xlsx']):
    c.execute(req_ecrit_MP, (int(tab['login'][i]), float(tab['600 (Mathématiques 1)'][i]), float(tab['601 ( Mathématiques 2)'][i]), float(tab['602 (Physique 1)'][i]), float(tab['603 (Physique 2)'][i]), float(tab['604 (Chimie)'][i]), float(tab['605 (Français)'][i]), float(tab['28 (Langue)'][i]), float(tab['1050 (Informatique)'][i]), float(tab['599 (Informatique ou Sciences industrielles)'][i]), float(tab['total_ecrit'][i])) )

    if tab['type_admissible'][i] == 'A': 
        if not math.isnan(tab['1 (QCM info/physique)'][i]):
            c.execute(req_oral_opt, (int(tab['login'][i]), float(tab['1 (QCM info/physique)'][i]), float(tab['3 (QCM anglais)'][i])) )

    else:
        c.execute(req_oral_B, (int(tab['login'][i]), float(tab['401 (Mathématiques (affichée))'][i]), float(tab['1 (QCM info/physique)'][i]), float(tab['2 (Entretien nouvelles technologies)'][i]), float(tab['3 (QCM anglais)'][i])) )

### TABLE Note_Ecrit_PC
tab = file['Classes_PC_CMT_spe_XXXX.xlsx']
for i in range(taille['Classes_PC_CMT_spe_XXXX.xlsx']):
    c.execute(req_ecrit_PC, (int(tab['login'][i]), float(tab['600 (Mathématiques 1)'][i]), float(tab['601 ( Mathématiques 2)'][i]), float(tab['602 (Physique 1)'][i]), float(tab['603 (Physique 2)'][i]), float(tab['604 (Chimie)'][i]), float(tab['605 (Français)'][i]), float(tab['28 (Langue)'][i]), float(tab['1050 (Informatique)'][i]), float(tab['total_ecrit'][i])) )
...
for i in range(taille['Classes_PSI_CMT_spe_XXXX.xlsx']):
    c.execute(req_ecrit_PSI, (int(tab['login'][i]), float(tab['600 (Mathématiques 1)'][i]), float(tab['601 ( Mathématiques 2)'][i]), float(tab['602 (Physique 1)'][i]), float(tab['603 (Physique 2)'][i]), float(tab['604 (Chimie)'][i]), float(tab['605 (Français)'][i]), float(tab['28 (Langue)'][i]), float(tab['1050 (Informatique)'][i]), float(tab['606 (Sciences industrielles)'][i]), float(tab['total_ecrit'][i])) )
...
for i in range(taille['Classes_PT_CMT_spe_XXXX.xlsx']):
    c.execute(req_ecrit_PT, (int(tab['login'][i]), float(tab['700 (Mathématiques B)'][i]), float(tab['701 (Mathématiques C)'][i]), float(tab['702 (Physique A)'][i]), float(tab['703 (Physique B)'][i]), float(tab['704 (Informatique modélisation)'][i]), float(tab['705 (Sciences industrielles A)'][i]), float(tab['706 (Français B)'][i]), float(tab['707 (Langue A)'][i]), float(tab['total_ecrit'][i])) )
...
for i in range(taille['Classes_TSI_CMT_spe_XXXX.xlsx']):
    c.execute(req_ecrit_TSI, (int(tab['login'][i]), float(tab['800 (Mathématiques 1)'][i]), float(tab['801 (Mathématiques 2)'][i]), float(tab['802 (Physique 1)'][i]), float(tab['803 (Physique 2)'][i]), float(tab['804 (Français)'][i]), float(tab['805 (Langue)'][i]), float(tab['806 (Sciences industrielles)'][i]), float(tab['807 (Informatique)'][i]), float(tab['total_ecrit'][i])) )
    c.execute(req_rang_ecrit, (int(tab['login'][i]), int(tab['rang_admissible'][i])) )
    if tab['type_admissible'][i] == 'A':
        c.execute(req_oral_A_TSI, (int(tab['login'][i]), float(tab['13 (Mathématiques 1)'][i]), float(tab['14 (Mathématiques 2)'][i]), float(tab['15 (Physique-chimie 1)'][i]), float(tab['16 (Physique-chimie 2)'][i]), float(tab['17 (Langue vivante)'][i]), float(tab['18 (TP Physique-chimie)'][i]), float(tab['19 (S2I)'][i])) )
    else:
        c.execute(req_oral_B, (int(tab['login'][i]), float(tab['4 (Mathématiques)'][i]), float(tab['1 (QCM info/physique)'][i]), float(tab['2 (Entretien nouvelles technologies)'][i]), float(tab['3 (QCM anglais)'][i])) )
...
for i in range(1,taille['ResultatOral_DD_MM_YYYY_ATS.xlsx']):
    ...
    if not math.isnan(tab['Rang'][i]):
        c.execute(req_resultat, (tab["Numéro d'inscription"][i], float(tab['Rang'][i]), float(tab['Total'][i]), float(tab['Moyenne'][i])) )
    else:
